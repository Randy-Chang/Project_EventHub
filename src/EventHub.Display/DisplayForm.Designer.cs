...
partial class DisplayForm
{
    ...
    private void InitializeComponent()
    {
        ...
        var distributionLabels = new[] { resultOptionALabel, resultOptionBLabel, resultOptionCLabel, resultOptionDLabel };
        var distributionBars = new[] { resultOptionABar, resultOptionBBar, resultOptionCBar, resultOptionDBar };
        var distributionCounts = new[] { resultOptionACountLabel, resultOptionBCountLabel, resultOptionCCountLabel, resultOptionDCountLabel };
        for (var index = 0; index < 4; index++)
        {
            distributionLabels[index].Dock = DockStyle.Fill;
            distributionLabels[index].Font = new Font("Microsoft JhengHei UI", 20F, FontStyle.Bold);
            distributionLabels[index].ForeColor = Color.White;
            distributionLabels[index].Text = $"{(char)('A' + index)}.";
            distributionLabels[index].TextAlign = ContentAlignment.MiddleLeft;
            distributionBars[index].Dock = DockStyle.Fill;
            distributionBars[index].Margin = new Padding(8, 22, 8, 22);
            distributionCounts[index].Dock = DockStyle.Fill;
            distributionCounts[index].Font = new Font("Microsoft JhengHei UI", 22F, FontStyle.Bold);
            distributionCounts[index].ForeColor = Color.White;
            distributionCounts[index].Text = "0";
            distributionCounts[index].TextAlign = ContentAlignment.MiddleRight;
            distributionTable.Controls.Add(distributionLabels[index], 0, index);
            distributionTable.Controls.Add(distributionBars[index], 1, index);
            distributionTable.Controls.Add(distributionCounts[index], 2, index);
        }
        //
        // leaderboardPanel
        //
